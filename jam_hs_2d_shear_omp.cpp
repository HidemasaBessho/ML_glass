...
void cell_list(int (*list)[Nn],double (*x)[dim],int M,double gamma)
{
  ...
  #pragma omp parallel for private(i,j,k,nx,ny,dx,dy,dy_temp,r2)
  for(i=0;i<Np;i++){
    list[i][0]=0;
    nx = f((int)((x[i][0]-gamma*x[i][1])*M/L),M);
    ny = f((int)(x[i][1]*M/L),M);
...
void calc_force_hs(double (*x)[dim],double (*f)[dim],double *a,double *U,int (*list)[Nn],double *txy,double *stress,double gamma){
  int i,j;
  double dx,dy,dy_temp,dr2,dUr,aij,dr,t;
  ini_matrix(f);
  ini_array(stress);
  *U=0.0;
  *txy=0.0;
  ...
  #pragma omp parallel for private(j,dx,dy,dy_temp,dr2,dUr,aij,dr) reduction(+:U0,rfxy0)
  for(i=0;i<Np;i++){
    for(j=1;j<=list[i][0];j++){
      ...
      if(dr2<aij*aij){
        ...
        rfxy0 += dx*dy/dr*dUr/L/L; //shear stress
...
int FIRE(double (*x)[dim],double (*f)[dim],double gamma,int (*list)[Nn],double *a,int M,double *U,double *txy,double *stress)
{
  ...
  for(;;){
    ...
    #pragma omp parallel for private(j,dx,dy,dy_temp)  reduction(+:P,sum_force,flag)
    for(i=0;i<Np;i++){
      for(j=0;j<dim;j++)
