...
set diameter(double *a,double r1,double r2){
  int i,p=0;
  for(i=0;i<Np;i++){
    if(p==0){
      a[i] = r1;
...
void cell_list(int (*list)[Nn],double (*x)[dim],int M,double Rcell)
{
  int i,j,k;
  int nx,ny,nz;
  int l,m,n;
...
void eom_langevin_hs(double (*v)[dim],double (*x)[dim],double (*f)[dim],int *a,double *U,double dt,int (*list)[Nn],double *kine){
  double zeta=1.0;
  *kine=0.0;
...
void auto_list_update(double *disp_max,double (*x)[dim],double (*x_update)[dim],int (*list)[Nn],int M,double Rcell){
  static int count=0;
  count++;
  calc_disp_max(&(*disp_max),x,x_update);
  if(*disp_max > skin*skin*0.25){
    cell_list(list,x,M,Rcell);
    update(x_update,x);
    *disp_max=0.0;
    count=0;
  }
...
void steepest_descent(double (*x)[dim],double (*f)[dim],int (*list)[Nn],double *a,int M,double *U){
...
int FIRE(double (*x)[dim],double (*f)[dim],int (*list)[Nn],double *a,int M,double *U)
{
  int i,j,imax;
  double alpha = 0.1,P;
  double dt0=0.0001;
...
int main()
{
  ...
  for(t=0.0;t<100.0;t+=dt){
    auto_list_update(&disp_max,x,x_update,list,M,Rcell);
    eom_langevin_hs(v,x,f,a,&U,dtbdhs,0.0,list,&kine);
  }
  
  auto_list_update(&disp_max,x,x_update,list,M,Rcell);
  
  steepest_descent(x,f,list,*a,M,&U);
  FIRE(x,f,list,a,M,&U);
